...
class SincConv_fast(nn.Module):
    ...
    def __init__(
        ...
        self.low_hz_ = nn.Parameter(
            torch.Tensor(hz[:-1]).view(-1, 1), requires_grad=requires_grad
        )

        # filter frequency band (out_channels, 1)
        self.band_hz_ = nn.Parameter(
            torch.Tensor(np.diff(hz)).view(-1, 1), requires_grad=requires_grad
        )
    ...
    def forward(self, waveforms):
        ...
        """
        Equivalent to Eq.4 of the reference paper (SPEAKER RECOGNITION FROM RAW WAVEFORM WITH SINCNET).
